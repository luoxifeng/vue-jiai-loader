...
module.exports = function jiaiLoader(source) {
  ...
  /**
   * 因为 jiai 是分析 template 的内容向 script 里注入代码
   * 所以只需要分拆出script块的时候才会处理
   */
  if (incomingQuery.type !== 'script') return source
  ...
  if (enforceUse) {
    let error = null
    if (script.src) {
      error = 'use jiai-loader can not use src in script'
    } else if (!template) { // template不存在
      error = 'use jiai-loader must have template'
    } else if (template.src) { // template 不能存在 src
      error = 'use jiai-loader can not use src in template'
    } else if (template.lang) { // template 不能存在 lang
      error = 'use jiai-loader can not use lang in template'
    }
    error && this.callback(new Error(error))
  } else {
